...
* Author: Alyssa Rose
* Date: 12-1-2018
* Notes: longest line is 110 characters, unable
* 	to fix do to the structure of the loops
*	and the conditionals for the move and merge
* Known Bugs: Will print out "Command not
* 	recognized" after using "show". I believe
* 	error is coming from the play() function
* 	reading in additional whitespace as a move
* 	and not registering is as a playable key
* Attempts to fix: tried using getline instead,
* 	attempted to read in both strings at once instead
* 	of waiting for the conditional but still had an
* 	error present
*/
...
/*
* prt_str()
* Purpose: prints starting message
*/
void Game::prt_start(){
...
* Returns: true if open space, false if full
*/
bool Game::filled(){
	for (int i = 0; i < Row; i++){
		for (int j = 0; j < Col; j++){
...
bool Game::make_mv(){
	...
	if ((board[Row-1][Col-1] == board[Row-2][Col-1]) or
		(board[Row-1][Col-1] == board[Row-1][Col-2]))
		return true;
	if ((board[Row-1][0] == board[Row-2][0]) or
		(board[Row-1][0] == board[Row-1][1]))
		return true;
	if ((board[0][0] == board[Row-2][0]) or
		(board[0][0] == board[Row-1][1]))
		return true;
	return false;
}
...
/*
* end_game()
* Purpose: ends the game when q pressed
* or moves not possible
*/
void Game::end_game(){
	string name;
...
/*
* play_other(string in)
* Purpose: handles non movement keys
*/
